<template>
    ...
    <div class="w-full text-center sm:w-[400px]">
      ...
      <h1 class="font-semibold text-3xl mt-4 mb-1">Welcome back</h1>
      <p class="text-sm text-stone-500 mb-5">Sign in to your MetricChat account</p>
      ...
      <form @submit.prevent="signInWithCredentials()" v-if="authMode !== 'sso_only'">
        <div class="field block mt-3">
          ...
          id='email'
          v-model='email'
          class="border border-stone-300 rounded-lg px-4 py-2 w-full h-10 text-sm focus:outline-none focus:border-primary-500"
          />
        </div>
        ...
        <div class="field mt-2 text-left" v-if="smtpEnabled">
          <NuxtLink to="/users/forgot-password" class="text-xs text-primary-500 hover:text-primary-700">
            Forgot Password?
          </NuxtLink>
        </div>
      ...
      New to MetricChat?
       <NuxtLink to="/users/sign-up" class="text-primary-500 hover:text-primary-700">
        Sign up
      </NuxtLink>
    </div>
    </div>
